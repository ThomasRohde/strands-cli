...
# Provide default API keys so provider creation in tests does not fail.
@pytest.fixture(autouse=True)
def _set_default_api_keys(monkeypatch: Any) -> None:
    """Set dummy provider API keys for tests.

    Individual tests can override or delete these values as needed.
    """
...
@pytest.fixture
def sample_openai_spec(sample_openai_spec_dict: dict[str, Any], monkeypatch: Any) -> Any:
    """Return a minimal OpenAI spec as a Spec object (mutable for tests).

    Sets a dummy OpenAI API key so provider creation in tests does not fail.
    """
    from strands_cli.types import Spec
