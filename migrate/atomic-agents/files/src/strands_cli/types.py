...
class Agent(BaseModel):
    ...
    Supports two modes:
    1. Inline definition: Specify prompt, tools, etc. directly
    2. Reference mode: Use $ref to reference an external atomic agent spec

    When using $ref, the referenced agent's configuration is loaded and merged
    with any overrides specified alongside the $ref.
    """

    # Reference to external atomic agent spec (alternative to inline definition)
    ref: str | None = Field(
        None,
        alias="$ref",
        description="Path to external atomic agent spec file (relative to current spec). "
        "When specified, agent definition is loaded from referenced file. "
        "Other fields can be specified to override referenced agent's settings.",
    )

    # Inline agent definition fields
    prompt: str | None = None  # Required if $ref not specified
    tools: list[str] | None = None  # Tool IDs to use
    model_id: str | None = None  # Override runtime model
    input_schema: str | dict[str, Any] | None = Field(
        None,
        description="Optional JSON Schema (path or inline object) used to validate inputs for this agent",
    )
    output_schema: str | dict[str, Any] | None = Field(
        None,
        description="Optional JSON Schema (path or inline object) used to validate outputs for this agent",
    )
    ...
    @model_validator(mode="after")
    def validate_ref_or_prompt(self) -> "Agent":
        """Ensure either $ref or prompt is specified (but not required for both)."""
        if self.ref is None and self.prompt is None:
            raise ValueError("Agent must specify either '$ref' or 'prompt'")
        return self

    model_config = {"populate_by_name": True}  # Allow both $ref and ref
