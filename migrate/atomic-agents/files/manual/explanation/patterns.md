...
## Patterns and Atomic Agents

[Atomic agents](atomic-agents.md) are single-purpose, reusable agents with explicit input/output contracts. They serve as **building blocks** for workflows across all patterns.

**How atomic agents integrate with patterns**:

### Chain Pattern
Use atomic agents as reusable steps:
```yaml
pattern:
  type: chain
  config:
...
Compose atomic agents into dependency graphs:
```yaml
pattern:
  type: workflow
  config:
...
agents:
  worker:
    $ref: ./agents/atomic/research_specialist.yaml#/agents/research_specialist
  orchestrator:
    prompt: "Break down research into subtasks"
...
**Benefits of using atomic agents in patterns**:
- ✅ **Reusability**: Write once, use in many workflows
- ✅ **Testability**: Each atomic agent has its own test suite
- ✅ **Discoverability**: `strands atomic list` shows available building blocks
- ✅ **Contracts**: Input/output schemas prevent composition errors
- ✅ **Performance**: Agent caching optimizes repeated use

**See Also**: [Atomic Agents Explanation](atomic-agents.md) for design philosophy and architecture.
...
- Build with atomic agents for maximum reusability and testability

**Next Steps**: See [Atomic Agents](atomic-agents.md), [Architecture Overview](architecture.md), [Design Decisions](design-decisions.md), [Performance Optimizations](performance.md), and [Security Model](security-model.md) for deeper dives into implementation details.
