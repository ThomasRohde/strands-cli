...
| `input_schema` | ❌ | string/object | JSON Schema for input validation |
| `output_schema` | ❌ | string/object | JSON Schema for output validation |

### Agent Reference with $ref (v0.6.0+)

Reference external atomic agents for true composition:

```yaml
agents:
  # Basic reference - inherits everything from atomic agent
  summarizer:
    $ref: ./agents/atomic/summarize_customer_email/summarize_customer_email.yaml
  
  # Reference with overrides
  classifier:
    $ref: ./agents/atomic/classify_ticket_priority/classify_ticket_priority.yaml
    model_id: gpt-4o  # Use larger model for this workflow
    tools: ["http_request"]  # Override tools
```

**Benefits**:
- Single source of truth (update atomic agent once, all workflows update)
- No duplication of prompts, schemas, or configurations
- Contract enforcement (schemas guaranteed to match)
- Override support for model, tools, and inference params

**Allowed overrides**: `model_id`, `provider`, `tools`, `inference` (temperature, top_p, max_tokens)

**See**: [Atomic Agents Tutorial](../tutorials/atomic-agents.md)
