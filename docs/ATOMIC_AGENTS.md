...
spec:
  ...
  input_schema:
    $ref: ./schemas/summarize_customer_email_input.json
  output_schema:
    $ref: ./schemas/summarize_customer_email_output.json
  tools: []
  inference:
    temperature: 0.2
```
...
./tests/
  summarize_customer_email_tests.yaml
```

Conventions:

- **Atomic Agents live in `./agents/atomic/` by default.**
- Other agent types are grouped by role under `./agents/orchestrators/`, `./agents/routers/`, etc.
- JSON Schemas go under `./schemas/` (or subfolders if needed).
- Test definitions go under `./tests/` (see §8).
...
```text
agents/atomic/summarize_customer_email.yaml
schemas/summarize_customer_email_input.json
schemas/summarize_customer_email_output.json
tests/summarize_customer_email_tests.yaml
```
...
- Location: `./tests/`
- Naming convention: `<agent_name>_tests.yaml`

Example: `tests/summarize_customer_email_tests.yaml`
...
Example orchestrator referencing atomic workers:
...
spec:
  ...
  orchestrator: agents/orchestrators/email_intake_orchestrator.yaml
  workers:
    - agents/atomic/summarize_customer_email.yaml
    - agents/atomic/classify_ticket_priority.yaml
```

- Atomic agents live as workers.
- The orchestrator remains thin and focused on routing, combining, and human-in-the-loop decisions.
...
spec:
  ...
  nodes:
    - id: summarize
      agent: agents/atomic/summarize_customer_email.yaml
    - id: classify
      agent: agents/atomic/classify_ticket_priority.yaml
  edges:
    - from: summarize
      to: classify
```
